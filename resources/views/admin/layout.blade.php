...
<body class="bg-slate-50 text-slate-800">
    ...
    <div class="min-h-screen flex">
        ...
        <aside
            ...
            $brandName = \App\Models\Setting::getVal('site_name', 'Data Greate VN');
            $brandParts = preg_split('/\s+/', $brandName, 2);
            $logoPath = \App\Models\Setting::getVal('site_logo', '');
            $lang = request('lang', 'vi');
            $langUrl = fn($code) => url()->current() . '?' . http_build_query(array_merge(request()->except('page'), ['lang' => $code]));
            ...
            <div class="px-4 pt-5 pb-3 border-b border-white/10 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    @else
                    <div
                        class="flex items-center justify-center w-10 h-10 rounded-full bg-[#ff8a00] text-black text-lg font-bold shadow-md">
                        {{ $brandInitial }}
                    </div>
                    @endif
            ...
            <div class="shrink-0 border-t border-white/10 bg-black/80">
                <div class="flex items-center justify-between px-4 py-3 text-white/90">
                    <div class="flex items-center gap-3 min-w-0">
                        <span
                            class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[#f4a90a] text-black text-sm font-bold shadow-md">
                            A
                        </span>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="h-14 bg-white border-b sticky top-0 z-20">
                <div class="h-full w-full px-3 sm:px-5 flex items-center justify-between gap-3">
                    ...
                    {{-- Right: Language dropdown (flag left + text right) --}}
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open"
                            class="flex items-center gap-2 px-3 py-1.5 rounded-md border border-[#ff8a00] text-[#ff8a00] font-medium text-sm bg-white hover:bg-orange-50 transition">
                            <img src="{{ asset($lang === 'en' ? 'images/flags/en.svg' : 'images/flags/vi.svg') }}"
                                class="w-5 h-5" alt="flag">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            class="absolute right-0 mt-2 w-44 bg-white text-gray-800 rounded-lg shadow-lg py-1 border border-[#ff8a00]/40 z-50">
                            @foreach ([ ['vi','Tiếng Việt','vi.svg'], ['en','English','en.svg'] ] as [$code,$label,$flag])
                            <a href="{{ $langUrl($code) }}"
                                class="flex items-center gap-2 px-3 py-1.5 hover:bg-orange-50 rounded
                                        {{ $lang === $code ? 'bg-[#fff7eb] border-l-2 border-[#ff8a00]' : '' }}">
                                <img src="{{ asset('images/flags/'.$flag) }}" class="w-5 h-5" alt="{{ $label }}">
                                <span>{{ $label }}</span>
                            </a>
                            @endforeach
                        </div>
