...
<div class="grid gap-4">
    <div>
        <label for="seo_default_title" class="block text-[13px] font-medium text-slate-700 mb-1">
            {{ t('admin.settings.meta.default_title','Default SEO Title') }}
        </label>
        <input id="seo_default_title" name="settings[seo.default_title]"
            ...
            class="w-full h-10 rounded-xl border border-slate-300/80 bg-white px-3 text-[14px]
                      placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#ff8a00]/40 focus:border-[#ff8a00]" />
    ...
    <div>
        <div class="flex items-center justify-between">
            <label for="seo_default_description" class="block text-[13px] font-medium text-slate-700 mb-1">
                {{ t('admin.settings.meta.default_desc','Default Meta Description') }}
            </label>
        ...
        <textarea id="seo_default_description" name="settings[seo.default_description]" rows="3"
            class="w-full rounded-xl border border-slate-300/80 bg-white px-3 py-2 text-[14px]
                         placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#ff8a00]/40 focus:border-[#ff8a00]">{{ old('settings.seo.default_description', setting('seo.default_description')) }}</textarea>
        @error('settings.seo.default_description')
        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        @enderror
        <p class="mt-1 text-[12px] text-slate-500">{{ t('admin.settings.meta.recommend','Khuyến nghị 120–160 ký tự.') }}</p>
    </div>
</div>
